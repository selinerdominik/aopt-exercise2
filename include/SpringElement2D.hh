...
//== CLASS DEFINITION =========================================================
    ...
    public:
        ...
        inline virtual void grad_f(const Vec &_x, const Vec &_coeffs, Vec &_g) override {
            ...
            _g[0] = _coeffs[0] * ((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3]) * (_x[1]-_x[3])) * 2 * (_x[0] - _x[2]);
            _g[1] = _coeffs[0] * ((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3]) * (_x[1]-_x[3])) * 2 * (_x[1] - _x[3]);
            _g[2] = - _coeffs[0] * ((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3]) * (_x[1]-_x[3])) * 2 * (_x[0] - _x[2]);
            _g[3] = - _coeffs[0] * ((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3]) * (_x[1]-_x[3])) * 2 * (_x[1] - _x[3]);
            //------------------------------------------------------//
        }
        ...
        inline virtual void hess_f(const Vec &_x, const Vec &_coeffs, Mat &_H) override {
            ...
            _H(0,0) = 2 * _coeffs[0]*(((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3])*(_x[1]-_x[3])) - _coeffs[1]*_coeffs[1])
                    + 4 * (_x[0]-_x[2]) *(_x[0]-_x[2]) * _coeffs[0];
            _H(1,0) = 4 * _coeffs[0]*(_x[0]-_x[2])*(_x[1]-_x[3]);
            _H(2,0) = - 2 * _coeffs[0]*(((_x[0]-_x[2])*(_x[0]-_x[2]) + (_x[1]-_x[3])*(_x[1]-_x[3])) - _coeffs[1] * _coeffs[1])
                    - 4 * (_x[0] - _x[2]) * (_x[0] - _x[2]) * _coeffs[0];
            _H(3,0) = - 4 * _coeffs[0]*(_x[0]-_x[2])*(_x[1]-_x[3]);
